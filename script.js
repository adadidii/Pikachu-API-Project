...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Membuat variable Flag/pengecek tanda. Berfungsi agar kartu Pokemon Card tidak terus menumpuk
  let isCardDisplayed = false; // Nilai dibuat false, dengan anggapan pada awalnya kartu memang belum muncul sebelum klik "Get"
  ...
  function showPikachuInfo() {
    if (isCardDisplayed === false) {
      ...
      `; // Inner HTML berfungsi untuk menambahkan suatu elemen, dengan format var.innerHTML = `<p>... ${var name}<p>`
          ...
          isCardDisplayed = true; // Variabel flag diubah menjadi true, karena sudah berhasil menampilkan pokemon card, sehingga mencegah duplikasi kartu, karena sudah bernilai true, bukan false lagi
        })
